window.FirmsUpListView = Backbone.View.extend({
    ...
    render: function () {
        ...
        $(this.el).append('<table id="table" class="highlight bordered"><thead>' +
            '<tr><td>Название</td><td>Дата Начала</td><td>Дата окончания</td><td>Тариф</td><td>Владелец</td><td>Статус</td><td>Действия</td></tr>' +
            '</thead></table>');

        for (var i = startPos; i < endPos; i++) {
            $('#table', this.el).append(new FirmsUpListItemView({model: firms[i], type: type}).render().el);
        }
...
window.FirmsUpListItemView = Backbone.View.extend({
