...
/**
 * Base class that represents a row from the 'stat' table.
 *
 *
 *
...
abstract class Stat implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\PropelModel\\Map\\StatTableMap';
    ...
    /**
     * The value for the fake_count field.
     * Note: this column has a database default value of: 0
     * @var        int
     */
    ...
    /**
     * Initializes internal state of PropelModel\Base\Stat object.
     * @see applyDefaults()
     */
     ...
     *
     * @return $this|Stat The current object, for fluid interface
     */
    ...
    /**
     * Get the [fake_count] column value.
     *
     * @return int
     */
    public function getFakeCount()
    {
        return $this->fake_count;
    }
     ...
     * @return $this|\PropelModel\Stat The current object (for fluent API support)
     */
    ...
    public function setEntityId($v)
    {
        ...
        if ($this->entity_id !== $v) {
            $this->entity_id = $v;
            $this->modifiedColumns[StatTableMap::COL_ENTITY_ID] = true;
        }
    ...
    public function setCityId($v)
    {
        ...
        if ($this->city_id !== $v) {
            $this->city_id = $v;
            $this->modifiedColumns[StatTableMap::COL_CITY_ID] = true;
        }
    ...
    public function setCount($v)
    {
        ...
        if ($this->count !== $v) {
            $this->count = $v;
            $this->modifiedColumns[StatTableMap::COL_COUNT] = true;
        }
    ...
    /**
     * Set the value of [fake_count] column.
     *
     * @param int $v new value
     * @return $this|\PropelModel\Stat The current object (for fluent API support)
     */
    public function setFakeCount($v)
    {
        if ($v !== null) {
            $v = (int) $v;
        }

        if ($this->fake_count !== $v) {
            $this->fake_count = $v;
            $this->modifiedColumns[StatTableMap::COL_FAKE_COUNT] = true;
        }
    ...
    public function setDate($v)
    {
        ...
        if ($this->date !== null || $dt !== null) {
            if ($this->date === null || $dt === null || $dt->format("Y-m-d") !== $this->date->format("Y-m-d")) {
                ...
                $this->modifiedColumns[StatTableMap::COL_DATE] = true;
            }
    ...
    public function setType($v)
    {
        ...
        if ($this->type !== $v) {
            $this->type = $v;
            $this->modifiedColumns[StatTableMap::COL_TYPE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : StatTableMap::translateFieldName('EntityId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->entity_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : StatTableMap::translateFieldName('CityId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->city_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : StatTableMap::translateFieldName('Count', TableMap::TYPE_PHPNAME, $indexType)];
            $this->count = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : StatTableMap::translateFieldName('FakeCount', TableMap::TYPE_PHPNAME, $indexType)];
            $this->fake_count = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : StatTableMap::translateFieldName('Date', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
            $this->date = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : StatTableMap::translateFieldName('Type', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 7; // 7 = StatTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\PropelModel\\Stat'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(StatTableMap::COL_ENTITY_ID)) {
            $modifiedColumns[':p' . $index++]  = 'entity_id';
        }
        if ($this->isColumnModified(StatTableMap::COL_CITY_ID)) {
            $modifiedColumns[':p' . $index++]  = 'city_id';
        }
        if ($this->isColumnModified(StatTableMap::COL_COUNT)) {
            $modifiedColumns[':p' . $index++]  = 'count';
        }
        if ($this->isColumnModified(StatTableMap::COL_FAKE_COUNT)) {
            $modifiedColumns[':p' . $index++]  = 'fake_count';
        }
        if ($this->isColumnModified(StatTableMap::COL_DATE)) {
            $modifiedColumns[':p' . $index++]  = 'date';
        }
        if ($this->isColumnModified(StatTableMap::COL_TYPE)) {
            $modifiedColumns[':p' . $index++]  = 'type';
        }

        $sql = sprintf(
            'INSERT INTO stat (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'count':
                        ...
                        break;
                    case 'fake_count':
                        $stmt->bindValue($identifier, $this->fake_count, PDO::PARAM_INT);
                        break;
                    case 'date':
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\PropelModel\Stat The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(StatTableMap::COL_ENTITY_ID)) {
            $criteria->add(StatTableMap::COL_ENTITY_ID, $this->entity_id);
        }
        if ($this->isColumnModified(StatTableMap::COL_CITY_ID)) {
            $criteria->add(StatTableMap::COL_CITY_ID, $this->city_id);
        }
        if ($this->isColumnModified(StatTableMap::COL_COUNT)) {
            $criteria->add(StatTableMap::COL_COUNT, $this->count);
        }
        if ($this->isColumnModified(StatTableMap::COL_FAKE_COUNT)) {
            $criteria->add(StatTableMap::COL_FAKE_COUNT, $this->fake_count);
        }
        if ($this->isColumnModified(StatTableMap::COL_DATE)) {
            $criteria->add(StatTableMap::COL_DATE, $this->date);
        }
        if ($this->isColumnModified(StatTableMap::COL_TYPE)) {
            $criteria->add(StatTableMap::COL_TYPE, $this->type);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \PropelModel\Stat (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setFakeCount($this->getFakeCount());
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \PropelModel\Stat Clone of current object.
     * @throws PropelException
     */
    ...
    {
        if (null !== $this->aRegion) {
            $this->aRegion->removeStat($this);
        }
        $this->id = null;
        $this->entity_id = null;
        $this->city_id = null;
        $this->count = null;
        $this->fake_count = null;
        $this->date = null;
        $this->type = null;
        $this->alreadyInSave = false;
        $this->clearAllReferences();
