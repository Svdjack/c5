...
abstract class Sessions implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\PropelModel\\Map\\SessionsTableMap';
     ...
     * Note: this column has a database default value of: 'undefined'
     * @var        string
     */
    protected $ip;
    ...
    /**
     * Initializes internal state of PropelModel\Base\Sessions object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\PropelModel\Sessions The current object (for fluent API support)
     */
    ...
    public function setValue($v)
    {
        ...
        if ($this->value !== $v) {
            $this->value = $v;
            $this->modifiedColumns[SessionsTableMap::COL_VALUE] = true;
        }
    ...
    public function setExpire($v)
    {
        ...
        if ($this->expire !== $v) {
            $this->expire = $v;
            $this->modifiedColumns[SessionsTableMap::COL_EXPIRE] = true;
        }
    ...
    public function setIp($v)
    {
        ...
        if ($this->ip !== $v) {
            $this->ip = $v;
            $this->modifiedColumns[SessionsTableMap::COL_IP] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : SessionsTableMap::translateFieldName('Value', TableMap::TYPE_PHPNAME, $indexType)];
            $this->value = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : SessionsTableMap::translateFieldName('Expire', TableMap::TYPE_PHPNAME, $indexType)];
            $this->expire = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : SessionsTableMap::translateFieldName('Ip', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 4; // 4 = SessionsTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\PropelModel\\Sessions'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(SessionsTableMap::COL_VALUE)) {
            $modifiedColumns[':p' . $index++]  = 'value';
        }
        if ($this->isColumnModified(SessionsTableMap::COL_EXPIRE)) {
            $modifiedColumns[':p' . $index++]  = 'expire';
        }
        if ($this->isColumnModified(SessionsTableMap::COL_IP)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\PropelModel\Sessions
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\PropelModel\Sessions The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(SessionsTableMap::COL_VALUE)) {
            $criteria->add(SessionsTableMap::COL_VALUE, $this->value);
        }
        if ($this->isColumnModified(SessionsTableMap::COL_EXPIRE)) {
            $criteria->add(SessionsTableMap::COL_EXPIRE, $this->expire);
        }
        if ($this->isColumnModified(SessionsTableMap::COL_IP)) {
            $criteria->add(SessionsTableMap::COL_IP, $this->ip);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \PropelModel\Sessions (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \PropelModel\Sessions Clone of current object.
     * @throws PropelException
     */
