...
/**
 * Base class that represents a row from the 'firm_contacts' table.
 *
 *
 *
...
abstract class Contact implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\PropelModel\\Map\\ContactTableMap';
    ...
    /**
     * Initializes internal state of PropelModel\Base\Contact object.
     */
     ...
     * @return $this|\PropelModel\Contact The current object (for fluent API support)
     */
    ...
    public function setFirmId($v)
    {
        ...
        if ($this->firm_id !== $v) {
            $this->firm_id = $v;
            $this->modifiedColumns[ContactTableMap::COL_FIRM_ID] = true;
        }
    ...
    public function setType($v)
    {
        ...
        if ($this->type !== $v) {
            $this->type = $v;
            $this->modifiedColumns[ContactTableMap::COL_TYPE] = true;
        }
    ...
    public function setValue($v)
    {
        ...
        if ($this->value !== $v) {
            $this->value = $v;
            $this->modifiedColumns[ContactTableMap::COL_VALUE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ContactTableMap::translateFieldName('FirmId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->firm_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ContactTableMap::translateFieldName('Type', TableMap::TYPE_PHPNAME, $indexType)];
            $this->type = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : ContactTableMap::translateFieldName('Value', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\PropelModel\\Contact'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ContactTableMap::COL_FIRM_ID)) {
            $modifiedColumns[':p' . $index++]  = 'firm_id';
        }
        if ($this->isColumnModified(ContactTableMap::COL_TYPE)) {
            $modifiedColumns[':p' . $index++]  = 'type';
        }
        if ($this->isColumnModified(ContactTableMap::COL_VALUE)) {
            $modifiedColumns[':p' . $index++]  = 'value';
        }

        $sql = sprintf(
            'INSERT INTO firm_contacts (%s) VALUES (%s)',
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\PropelModel\Contact
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\PropelModel\Contact The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ContactTableMap::COL_FIRM_ID)) {
            $criteria->add(ContactTableMap::COL_FIRM_ID, $this->firm_id);
        }
        if ($this->isColumnModified(ContactTableMap::COL_TYPE)) {
            $criteria->add(ContactTableMap::COL_TYPE, $this->type);
        }
        if ($this->isColumnModified(ContactTableMap::COL_VALUE)) {
            $criteria->add(ContactTableMap::COL_VALUE, $this->value);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \PropelModel\Contact (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \PropelModel\Contact Clone of current object.
     * @throws PropelException
     */
