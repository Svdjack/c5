...
use \Exception;
use \PDO;
use PropelModel\AdminQuery as ChildAdminQuery;
use PropelModel\Map\AdminTableMap;
...
abstract class Admin implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\PropelModel\\Map\\AdminTableMap';
    ...
    /**
     * Initializes internal state of PropelModel\Base\Admin object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\PropelModel\Admin The current object (for fluent API support)
     */
    ...
    public function setLogin($v)
    {
        ...
        if ($this->login !== $v) {
            $this->login = $v;
            $this->modifiedColumns[AdminTableMap::COL_LOGIN] = true;
        }
    ...
    public function setLastLogin($v)
    {
        ...
        if ($this->last_login !== $v) {
            $this->last_login = $v;
            $this->modifiedColumns[AdminTableMap::COL_LAST_LOGIN] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : AdminTableMap::translateFieldName('Login', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : AdminTableMap::translateFieldName('LastLogin', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\PropelModel\\Admin'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(AdminTableMap::COL_LOGIN)) {
            $modifiedColumns[':p' . $index++]  = 'login';
        }
        if ($this->isColumnModified(AdminTableMap::COL_HASH)) {
            $modifiedColumns[':p' . $index++]  = 'hash';
        }
        if ($this->isColumnModified(AdminTableMap::COL_LAST_LOGIN)) {
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\PropelModel\Admin The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(AdminTableMap::COL_LOGIN)) {
            $criteria->add(AdminTableMap::COL_LOGIN, $this->login);
        }
        if ($this->isColumnModified(AdminTableMap::COL_HASH)) {
            $criteria->add(AdminTableMap::COL_HASH, $this->hash);
        }
        if ($this->isColumnModified(AdminTableMap::COL_LAST_LOGIN)) {
            $criteria->add(AdminTableMap::COL_LAST_LOGIN, $this->last_login);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \PropelModel\Admin (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \PropelModel\Admin Clone of current object.
     * @throws PropelException
     */
