...
/**
 * Base class that represents a row from the 'firm_childs' table.
 *
 *
 *
...
abstract class Child implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\PropelModel\\Map\\ChildTableMap';
    ...
    /**
     * Initializes internal state of PropelModel\Base\Child object.
     */
     ...
     * @return $this|\PropelModel\Child The current object (for fluent API support)
     */
    public function setid($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[ChildTableMap::COL_ID] = true;
        }
    ...
    public function setFirmId($v)
    {
        ...
        if ($this->firm_id !== $v) {
            $this->firm_id = $v;
            $this->modifiedColumns[ChildTableMap::COL_FIRM_ID] = true;
        }
    ...
    public function setValue($v)
    {
        ...
        if ($this->value !== $v) {
            $this->value = $v;
            $this->modifiedColumns[ChildTableMap::COL_VALUE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : ChildTableMap::translateFieldName('id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ChildTableMap::translateFieldName('FirmId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->firm_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ChildTableMap::translateFieldName('Value', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = ChildTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\PropelModel\\Child'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[ChildTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . ChildTableMap::COL_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(ChildTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(ChildTableMap::COL_FIRM_ID)) {
            $modifiedColumns[':p' . $index++]  = 'firm_id';
        }
        if ($this->isColumnModified(ChildTableMap::COL_VALUE)) {
            $modifiedColumns[':p' . $index++]  = 'value';
        }

        $sql = sprintf(
            'INSERT INTO firm_childs (%s) VALUES (%s)',
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\PropelModel\Child The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ChildTableMap::COL_ID)) {
            $criteria->add(ChildTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(ChildTableMap::COL_FIRM_ID)) {
            $criteria->add(ChildTableMap::COL_FIRM_ID, $this->firm_id);
        }
        if ($this->isColumnModified(ChildTableMap::COL_VALUE)) {
            $criteria->add(ChildTableMap::COL_VALUE, $this->value);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \PropelModel\Child (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \PropelModel\Child Clone of current object.
     * @throws PropelException
     */
