...
abstract class Tags implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\PropelModel\\Map\\TagsTableMap';
    ...
    /**
     * Initializes internal state of PropelModel\Base\Tags object.
     */
     ...
     * @return $this|\PropelModel\Tags The current object (for fluent API support)
     */
    ...
    public function setTag($v)
    {
        ...
        if ($this->tag !== $v) {
            $this->tag = $v;
            $this->modifiedColumns[TagsTableMap::COL_TAG] = true;
        }
    ...
    public function setUrl($v)
    {
        ...
        if ($this->url !== $v) {
            $this->url = $v;
            $this->modifiedColumns[TagsTableMap::COL_URL] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : TagsTableMap::translateFieldName('Tag', TableMap::TYPE_PHPNAME, $indexType)];
            $this->tag = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : TagsTableMap::translateFieldName('Url', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\PropelModel\\Tags'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(TagsTableMap::COL_TAG)) {
            $modifiedColumns[':p' . $index++]  = 'tag';
        }
        if ($this->isColumnModified(TagsTableMap::COL_URL)) {
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\PropelModel\Tags The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(TagsTableMap::COL_TAG)) {
            $criteria->add(TagsTableMap::COL_TAG, $this->tag);
        }
        if ($this->isColumnModified(TagsTableMap::COL_URL)) {
            $criteria->add(TagsTableMap::COL_URL, $this->url);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \PropelModel\Tags (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \PropelModel\Tags Clone of current object.
     * @throws PropelException
     */
     ...
     * @return ObjectCollection|ChildFirmTags[] List of ChildFirmTags objects
     */
    public function getFirmTagssJoinFirm(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildFirmTagsQuery::create(null, $criteria);
