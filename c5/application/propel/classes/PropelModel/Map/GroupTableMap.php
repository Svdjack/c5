...
class GroupTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'PropelModel.Map.GroupTableMap';
     ...
     */
    const OM_CLASS = '\\PropelModel\\Group';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'PropelModel.Group';
     ...
     */
    const COL_PARENT = 'groups.parent';

    /**
     * the column name for the level field
     */
    const COL_LEVEL = 'groups.level';
     ...
     */
    const COL_ORIGINAL = 'groups.original';

    /**
     * the column name for the url field
     */
    const COL_URL = 'groups.url';

    /**
     * the column name for the worktime field
     */
    const COL_WORKTIME = 'groups.worktime';

    /**
     * the column name for the live field
     */
    const COL_LIVE = 'groups.live';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('Id', 'Parent', 'Level', 'Name', 'Original', 'Url', 'Worktime', 'Live', ),
        self::TYPE_CAMELNAME     => array('id', 'parent', 'level', 'name', 'original', 'url', 'worktime', 'live', ),
        self::TYPE_COLNAME       => array(GroupTableMap::COL_ID, GroupTableMap::COL_PARENT, GroupTableMap::COL_LEVEL, GroupTableMap::COL_NAME, GroupTableMap::COL_ORIGINAL, GroupTableMap::COL_URL, GroupTableMap::COL_WORKTIME, GroupTableMap::COL_LIVE, ),
        self::TYPE_FIELDNAME     => array('id', 'parent', 'level', 'name', 'original', 'url', 'worktime', 'live', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('Id' => 0, 'Parent' => 1, 'Level' => 2, 'Name' => 3, 'Original' => 4, 'Url' => 5, 'Worktime' => 6, 'Live' => 7, ),
        self::TYPE_CAMELNAME     => array('id' => 0, 'parent' => 1, 'level' => 2, 'name' => 3, 'original' => 4, 'url' => 5, 'worktime' => 6, 'live' => 7, ),
        self::TYPE_COLNAME       => array(GroupTableMap::COL_ID => 0, GroupTableMap::COL_PARENT => 1, GroupTableMap::COL_LEVEL => 2, GroupTableMap::COL_NAME => 3, GroupTableMap::COL_ORIGINAL => 4, GroupTableMap::COL_URL => 5, GroupTableMap::COL_WORKTIME => 6, GroupTableMap::COL_LIVE => 7, ),
        self::TYPE_FIELDNAME     => array('id' => 0, 'parent' => 1, 'level' => 2, 'name' => 3, 'original' => 4, 'url' => 5, 'worktime' => 6, 'live' => 7, ),
        self::TYPE_NUM           => array(0, 1, 2, 3, 4, 5, 6, 7, )
    );
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\PropelModel\\Group');
        $this->setPackage('PropelModel');
        $this->setUseIdGenerator(true);
        // columns
        $this->addPrimaryKey('id', 'Id', 'INTEGER', true, 10, null);
        $this->addColumn('parent', 'Parent', 'INTEGER', true, null, 0);
        $this->addColumn('level', 'Level', 'INTEGER', true, null, 1);
        $this->addColumn('name', 'Name', 'VARCHAR', true, 255, '');
        $this->addColumn('original', 'Original', 'VARCHAR', true, 255, '');
        $this->addColumn('url', 'Url', 'VARCHAR', true, 255, null);
        $this->addColumn('worktime', 'Worktime', 'VARBINARY', false, null, null);
        $this->addColumn('live', 'Live', 'LONGVARCHAR', false, null, null);
    } // initialize()
  ...
  array (
    ...
    1 => ':id',
        ...
        $this->addRelation('Firm', '\\PropelModel\\Firm', RelationMap::MANY_TO_MANY, array(), 'CASCADE', null, 'Firms');
    } // buildRelations()
    /**
    ...
    public static function addSelectColumns(Criteria $criteria, $alias = null)
    {
        if (null === $alias) {
            ...
            $criteria->addSelectColumn(GroupTableMap::COL_PARENT);
            $criteria->addSelectColumn(GroupTableMap::COL_LEVEL);
            $criteria->addSelectColumn(GroupTableMap::COL_NAME);
            $criteria->addSelectColumn(GroupTableMap::COL_ORIGINAL);
            $criteria->addSelectColumn(GroupTableMap::COL_URL);
            $criteria->addSelectColumn(GroupTableMap::COL_WORKTIME);
            $criteria->addSelectColumn(GroupTableMap::COL_LIVE);
        } else {
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \PropelModel\Group) { // it's a model object
