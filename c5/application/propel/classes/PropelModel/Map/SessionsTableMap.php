...
use PropelModel\Sessions;
use PropelModel\SessionsQuery;
...
class SessionsTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'PropelModel.Map.SessionsTableMap';
     ...
     */
    const OM_CLASS = '\\PropelModel\\Sessions';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'PropelModel.Sessions';
     ...
     */
    const COL_VALUE = 'sessions.value';

    /**
     * the column name for the expire field
     */
    const COL_EXPIRE = 'sessions.expire';

    /**
     * the column name for the ip field
     */
    const COL_IP = 'sessions.ip';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('Id', 'Value', 'Expire', 'Ip', ),
        self::TYPE_CAMELNAME     => array('id', 'value', 'expire', 'ip', ),
        self::TYPE_COLNAME       => array(SessionsTableMap::COL_ID, SessionsTableMap::COL_VALUE, SessionsTableMap::COL_EXPIRE, SessionsTableMap::COL_IP, ),
        self::TYPE_FIELDNAME     => array('id', 'value', 'expire', 'ip', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('Id' => 0, 'Value' => 1, 'Expire' => 2, 'Ip' => 3, ),
        self::TYPE_CAMELNAME     => array('id' => 0, 'value' => 1, 'expire' => 2, 'ip' => 3, ),
        self::TYPE_COLNAME       => array(SessionsTableMap::COL_ID => 0, SessionsTableMap::COL_VALUE => 1, SessionsTableMap::COL_EXPIRE => 2, SessionsTableMap::COL_IP => 3, ),
        self::TYPE_FIELDNAME     => array('id' => 0, 'value' => 1, 'expire' => 2, 'ip' => 3, ),
        self::TYPE_NUM           => array(0, 1, 2, 3, )
    );
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\PropelModel\\Sessions');
        $this->setPackage('PropelModel');
        $this->setUseIdGenerator(false);
        // columns
        $this->addPrimaryKey('id', 'Id', 'VARCHAR', true, 32, '');
        $this->addColumn('value', 'Value', 'VARCHAR', true, 255, '');
        $this->addColumn('expire', 'Expire', 'INTEGER', true, null, null);
        $this->addColumn('ip', 'Ip', 'VARCHAR', true, 15, 'undefined');
    } // initialize()
    ...
    public static function addSelectColumns(Criteria $criteria, $alias = null)
    {
        if (null === $alias) {
            ...
            $criteria->addSelectColumn(SessionsTableMap::COL_VALUE);
            $criteria->addSelectColumn(SessionsTableMap::COL_EXPIRE);
            $criteria->addSelectColumn(SessionsTableMap::COL_IP);
        } else {
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \PropelModel\Sessions) { // it's a model object
